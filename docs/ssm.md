...
└── clients/
    └── {client_slug}/
        └── {agent_slug}                                  — agent config record (SecureString JSON)
            ...
            ├── /gateway_token                            — SecureString, managed by tofu
...
| `bedrock_model` | string | no | `bedrock/us.anthropic.claude-haiku-4-5-20251001-v1:0` | Bedrock model string, passed as `OPENCLAW_MODEL` env var |
...
### `/clawless/clients/{client_slug}/{agent_slug}/gateway_token`

| | |
|---|---|
| Type | SecureString |
| Purpose | Shared token for authenticating WebSocket connections to the gateway (e.g. from the OpenClaw CLI) |
| Writers | Tofu — generated once per agent via `random_password`, with `ignore_changes = [value]` so it persists across applies |
| Readers | Fargate execution role (via task def `secrets[]`, injected as `OPENCLAW_GATEWAY_TOKEN` env var) |
| IAM | Execution role: `ssm:GetParameters` on `/clawless/clients/*/gateway_token` |

See [credentials.md](credentials.md) for the full delivery path.
...
| Purpose | Debug toggle — when present, the gateway entrypoint appends `--verbose` to `openclaw gateway run`, sending verbose logs to stdout/stderr (and thence to CloudWatch) |
...
| Fargate execution role | `ssm:GetParameters` | `/clawless/clients/*/gateway_token` |
