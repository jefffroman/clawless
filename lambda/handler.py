...
MAX_RECHECK_ITERATIONS = 10        # genuine flapping past this warrants an alert
TIME_REMAINING_FLOOR_MS = 120_000  # 2 min — enough for one tofu apply
...
class _IntentChanged(Exception):
    """Raised by _release() when a new event arrived during processing.

    Carries the freshly-read DDB item so the caller can extract the new
    timestamp and re-dispatch without an extra read.
    """
...
def lambda_handler(event, context):
    ...
    # ── Step 5: RECHECK + RELEASE ─────────────────────────────────────
    # If a new event arrived while we owned the slug, SFN's CheckInProgress
    # branch skipped Lambda invocation (we held in_progress=true). The
    # conditional delete in _release() detects this atomically: on
    # ConditionalCheckFailedException we re-read SSM, redispatch this slug,
    # and retry. Bounded by iteration count and remaining Lambda time.
    for slug, info in grabbed.items():
        current_ts = info["timestamp"]
        for i in range(MAX_RECHECK_ITERATIONS):
            if context.get_remaining_time_in_millis() < TIME_REMAINING_FLOOR_MS:
                _send_alert(
                    f"Lifecycle recheck: time floor for {slug}",
                    f"Bailing after {i} recheck iter(s); force-deleting record.",
                )
                dynamodb.delete_item(
                    TableName=LIFECYCLE_TABLE, Key={"slug": {"S": slug}}
                )
                break
            try:
                _release(slug, current_ts)
                break
            except _IntentChanged as e:
                new_ts = e.item.get("timestamp", {}).get("S", "")
                print(f"[recheck:{slug}] iter {i+1}: intent {current_ts} -> {new_ts}")
                agents = _get_agents()
                errored_slugs = {s for s, c in agents.items() if c.get("_error")}
                if not _dispatch_slug(slug, agents, version, errored_slugs):
                    # /error set mid-recheck — operator must clear before resume
                    print(f"[recheck:{slug}] /error set — surrendering ownership")
                    dynamodb.delete_item(
                        TableName=LIFECYCLE_TABLE, Key={"slug": {"S": slug}}
                    )
                    break
            ...
            _send_alert(
                f"Lifecycle recheck exceeded {MAX_RECHECK_ITERATIONS} iter for {slug}",
                "Possible runaway intent flapping; force-deleting record.",
            )
            dynamodb.delete_item(
                TableName=LIFECYCLE_TABLE, Key={"slug": {"S": slug}}
            )
...
def _release(slug, grabbed_timestamp):
    """Release ownership via conditional delete.

    On timestamp mismatch, raises _IntentChanged carrying the freshly-read
    item so the caller can extract the new timestamp and redispatch. SFN
    skipped Lambda invocation for the racing event (we held in_progress=true),
    so the owning Lambda must reconcile the new intent itself.
    """
    if not LIFECYCLE_TABLE:
        return
    ...
    try:
        ...
        raise _IntentChanged(resp.get("Item", {})) from e
...
def _dispatch_slug(slug, agents, version, errored_slugs):
    """Per-slug redispatch for the recheck path. Mirrors the four transitions
    in lambda_handler step 2-4 but for one slug at a time.

    Returns False if the slug is in /error state and the caller should
    surrender ownership; True if dispatch was attempted.
    """
    if slug in errored_slugs:
        return False
    has_service = _fargate_service_exists(slug)
    if slug not in agents:                              # remove
        if has_service:
            _fargate_set_desired(slug, 0)
        _apply_with_retry(version, agents, {slug}, errored_slugs)
    elif not agents[slug].get("active", True):         # pause
        if has_service:
            _fargate_set_desired(slug, 0)
    elif has_service:                                   # resume
        _fargate_wake(slug)
    else:                                               # add
        _apply_with_retry(version, agents, {slug}, errored_slugs)
    return True
