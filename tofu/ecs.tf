...
# Allow the execution role to read the gateway token from SSM SecureString.
resource "aws_iam_role_policy" "fargate_execution_ssm" {
  name = "ssm-secrets"
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = "arn:aws:ssm:${var.aws_region}:*:parameter/clawless/clients/*/gateway_token"
    }]
  })
}
