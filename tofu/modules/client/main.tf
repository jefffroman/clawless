...
resource "aws_ecs_task_definition" "gateway" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "CLAWLESS_MODEL", value = var.bedrock_model },
      { name = "CLAWLESS_CHANNEL", value = var.agent_channel },
      {
        name  = "CLAWLESS_CHANNEL_CONFIG"
    ...
    # Sync_up to S3 happens during shutdown; give it headroom over the prior
    # OpenClaw default (30 s) so pathologically large workspaces don't get
    # truncated mid-sync.
    stopTimeout = 45
  }])
