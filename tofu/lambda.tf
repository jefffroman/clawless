...
# Lightweight Lambda that receives Telegram webhook POSTs for paused agents.
# Queues the wake message in DynamoDB, sets /active=true, and triggers the
# lifecycle SFN to resume the instance.  Single shared Lambda for all agents;
# the agent slug is encoded in Telegram's secret_token header.
