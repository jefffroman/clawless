  ...
  definition = jsonencode({
    ...
    States = {
      ...
      WritePending = {
        ...
        Next       = "CheckIfExisting"
      }
      CheckIfExisting = {
        # ALL_OLD omits Attributes when the UpdateItem created the record. If
        # Attributes is present, a record already existed — meaning a Lambda
        # has been (or will be) invoked for it. That Lambda reconciles our
        # pending+ts update via the release-recheck loop; skip a redundant
        # invocation. Closes the SFN→Lambda gap that otherwise lets two
        # Lambdas spawn for back-to-back events.
        Type = "Choice"
        Choices = [{
          Variable  = "$.dynamoResult.Attributes"
          IsPresent = true
          Next      = "AlreadyOwned"
        }]
      ...
      AlreadyOwned = {
        Type    = "Succeed"
        Comment = "Record already existed — a Lambda is already on the way"
      }
      InvokeLambda = {
        Type     = "Task"
